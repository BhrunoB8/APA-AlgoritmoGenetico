...
mapeamento_normalizado = colors.Normalize(
    vmin=0, vmax=19)  # mapeamento em 20 cores
mapa_escalar = cmx.ScalarMappable(
    norm=mapeamento_normalizado, cmap=mapa_cor)  # lista de cores final
...
class Experimento(ABC):
    ...
    def imprime_dados(self):
        ...
        for i in range(len(self.tamanhos)):
            print("%03d\t%02f\t%02f\t%02f" % (
                self.tamanhos[i], self.medias[i], self.desvios[i], self.aproximados[i]))
        print("")
    ...
    def plota_aproximacao(self):
        plt.plot(self.tamanhos_aproximados, self.aproximados,
                 label=self.aproximacao_legenda, color=self.aproximacao_cor_rgb)
    ...
    def plota_gn(self, constante, legenda, cor, estilo_linha):
        ...
        plt.plot(self.tamanhos_aproximados, valores,
                 linestyle=estilo_linha, label=legenda, color=cor)

    def plota_assintotica(self):
        if self.gn1_constante is not None:
            self.plota_gn(self.gn1_constante, self.gn1_legenda,
                          self.aproximacao_cor_rgb, "dotted")
        if self.gn2_constante is not None:
            self.plota_gn(self.gn2_constante, self.gn2_legenda,
                          self.aproximacao_cor_rgb, "dashed")


class TspGeneticAlgorithm(Experimento):

    def __init__(self, args):
        super().__init__(args)
        self.id = "t"
        self.script = "alg_tsp_genetic_algorithm.py"
        self.output = "out_tsp_genetic_algorithm.txt"

        indice_cor = 1

        # configurações de plotagem
        self.medicao_legenda = "tsp genetic algorithm"
        self.medicao_cor_rgb = mapa_escalar.to_rgba(2*indice_cor)
        self.medicao_formato = formatos[indice_cor]

        self.aproximacao_legenda = "tsp genetic algorithm"
        self.aproximacao_cor_rgb = mapa_escalar.to_rgba(2*indice_cor+1)
    ...
    def executa_aproximacao(self):
        ...
        parametros, pcov = opt.curve_fit(
            funcao_quadratica, xdata=self.tamanhos, ydata=self.medias)
        self.aproximados = [funcao_quadratica(
            x, *parametros) for x in self.tamanhos_aproximados]
...
# cristiano ronaldo chegou a marca de 800 gols na carreira hoje
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Avaliação Analítica')

    help_msg = "semente aleatória"
    parser.add_argument("--seed", "-s", help=help_msg,
                        default=DEFAULT_SEED, type=int)

    help_msg = "n stop.          Padrão:{}".format(DEFAULT_N_STOP)
    parser.add_argument("--nstop", "-n", help=help_msg,
                        default=DEFAULT_N_STOP, type=int)

    help_msg = "n mínimo.          Padrão:{}".format(DEFAULT_N_START)
    parser.add_argument("--nstart", "-a", help=help_msg,
                        default=DEFAULT_N_START, type=int)

    help_msg = "n passo.           Padrão:{}".format(DEFAULT_N_STEP)
    parser.add_argument("--nstep", "-e", help=help_msg,
                        default=DEFAULT_N_STEP, type=int)

    help_msg = "n máximo.          Padrão:{}".format(DEFAULT_N_MAX)
    parser.add_argument("--nmax", "-m", help=help_msg,
                        default=DEFAULT_N_MAX, type=int)

    help_msg = "tentativas.        Padrão:{}".format(DEFAULT_TRIALS)
    parser.add_argument("--trials", "-t", help=help_msg,
                        default=DEFAULT_TRIALS, type=int)

    help_msg = "figura (extensão .png ou .pdf) ou nenhum para apresentar na tela.  Padrão:{}".format(
        DEFAULT_OUTPUT)
    parser.add_argument("--out", "-o", help=help_msg,
                        default=DEFAULT_OUTPUT, type=str)

    help_msg = "algoritmos (t=tsp genetic algorithm) ou nenhum para executar todos.  Padrão:{}".format(
        DEFAULT_ALGORITMOS)
    parser.add_argument("--algoritmos", "-l", help=help_msg,
                        default=DEFAULT_ALGORITMOS, type=str)

    help_msg = "verbosity logging level (INFO=%d DEBUG=%d)" % (
        logging.INFO, logging.DEBUG)
    parser.add_argument("--verbosity", "-v", help=help_msg,
                        default=DEFAULT_LOG_LEVEL, type=int)

    # action='store_true'
    parser.add_argument("--skip", "-k", default=False,
                        help="Pula execução.", action='store_true')
    ...
    experimentos = [TspGeneticAlgorithm(args)]
    ...
    plt.title("Analítica - Algoritmo Genético")
